...
function LandingPage() {
    // 1. Start with 'null' so no form is visible initially
  ...
  return (
    <div className='flex flex-col items-center justify-center min-h-[60vh] p-10'>
        ...
        {/* 2. Show this ONLY if no role is selected (!view) */}
        {!view && (
            <div className="flex flex-col items-center gap-10 animate-fadeIn">
                ...
                <div className='flex flex-wrap justify-center gap-8'>
                    <button className={btnStyle} onClick={() => setView('Student')}>Student</button>
                    <button className={btnStyle} onClick={() => setView('Warden')}>Warden</button>
                    <button className={btnStyle} onClick={() => setView('Admin')}>Admin</button>
                </div>
            </div>
        )}

        {/* 3. Show the Form and a "Back" button if a role IS selected (view) */}
        {view && (
            <div className="w-full flex flex-col items-center gap-6 animate-slideUp">
                ...
                {/* The "Back" button to bring the role buttons back */}
                <button 
                    onClick={() => setView(null)}
                    className="flex items-center gap-2 text-slate-400 font-bold hover:text-blue-700 transition-colors uppercase text-xs tracking-widest"
                >
